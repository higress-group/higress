...
这不是一套独立的“报告环境”。它是 Higress 默认 CI/CD 流水线中的常规集成测试 job，沿用仓库已有名称 `gateway-conformance-test`，与现有 `higress-conformance-test` 和 `higress-wasmplugin-test` 同级。每个 job 按现有模式创建自己的临时 kind，测试结束即清理，不引入长期集群或人工执行环境。
...
- `.github/workflows/build-and-test.yaml` 原先预留了空的 `gateway-conformance-test` job，现已直接补全实现。
- 该 workflow 已对所有目标分支的 `pull_request` 触发，因此补全 job 后无需新增 workflow 或额外触发器。

### 3.2 实施前缺口

1. `Makefile.core.mk` 原有的空 `gateway-conformance-test` target 已直接补全实现。
2. CI 的同名 job 只有 checkout，是“空成功”。
3. `test/gateway/e2e.go` 和 `test/gateway/e2e_test.go` 没有测试逻辑。
4. 测试环境没有显式安装官方 v1.4.0 Standard CRD。
5. `GatewayClass.status.supportedFeatures` 为空；v1.4 Suite 在未传 features 时会尝试从该字段推断。为避免仅为测试改变现有用户可见 status，测试入口需要显式传入首期 Core features。
6. `pkg/ingress/kube/gateway/istio/supported_features.go` 现有值为 `features.AllFeatures`，但本次 v1.4 Core 接入不依赖或修改它。
...
`gateway-conformance-test` 必须放在现有 `.github/workflows/build-and-test.yaml` 中，不新建只供人工运行的申报 workflow。它与 `higress-conformance-test` 一样依赖 `build`，在每个 PR 中默认执行：
...
lint + coverage
       ...
       build
        |-------------------------------|
        |                               |
higress-conformance-test      gateway-conformance-test
                                        |
...
- `test/gateway/` + `make gateway-conformance-test`。

默认 PR job 的成功条件就是完整 GATEWAY-HTTP Core 通过。它不能以“只生成报告”“允许失败”或“仅定时运行”替代 PR 门禁。仓库启用 required checks 后，应将 `gateway-conformance-test` 列为保护分支必需检查。
...
v1.4.0 官方 runner 支持 `--supported-features`，官方文档和该版本自带报告示例也使用这一参数。默认 PR 与正式报告均固定传入 `Gateway,HTTPRoute,ReferenceGrant`，其含义是选择 `GATEWAY-HTTP` 的 Core 测试范围，不是跳过失败用例。

本次不修改 `GatewayClass.status.supportedFeatures` 和 `supported_features.go`，避免为了测试改变现有用户可见状态。仍然不使用 `--all-features`、`--skip-tests`、`--exempt-features` 或 `--allow-crds-mismatch`。开发者可额外使用 `--run-test=<ShortName>` 做单项调试，但完整 PR job 始终执行全部 33 个 Core 测试。

### 4.5 一套测试入口，两种报告元数据

PR 回归和官方申报必须调用同一个 `make gateway-conformance-test`，不能维护两套 runner 或两套环境。差异只在 `implementation.version`：
...
保留 `pkg/ingress/kube/gateway/istio/supported_features.go`、`GatewayClass.status.supportedFeatures` 和相关 golden 的原有行为。首期测试范围只在 Make target 中通过 `GATEWAY_CONFORMANCE_SUPPORTED_FEATURES=Gateway,HTTPRoute,ReferenceGrant` 声明。

### 5.3 `GatewayClass` 必要修复

官方要求实现监听所有 GatewayClass，并处理 `spec.controllerName` 匹配的对象，而不是只处理固定名称 `higress`。本次只把 GatewayClass 归属判断改为 controllerName，并增加 alternate class 单元测试；不增加其他 status 字段。
...
- `gateway-conformance-test-prepare`：delete/create kind、安装 CRD、构建/加载当前提交的 Higress controller、加载仓库统一的 pilot/gateway 镜像、Helm install、等待 workloads 和 GatewayClass Accepted。Gateway API job 不维护自己的数据面 tag；若改动涉及 pilot，先构建并推送对应 submodule SHA 的 pilot 镜像，再统一更新 `ISTIO_LATEST_IMAGE_TAG`，让现有 e2e 与 Gateway API e2e 使用同一产物。
- `run-gateway-conformance-test`：执行完整官方 GATEWAY-HTTP Core 并生成报告；这是 PR 默认路径。
- `gateway-conformance-test-clean`：删除 kind。
- `gateway-conformance-test`：串联 prepare/run；失败时由 CI `always()` 收集诊断后清理。
...
PR 默认命令和正式申报命令均显式传入三个 Core features，并且故意没有：
...
4. 显式以 PR head SHA 作为 checkout ref、镜像 `TAG` 和 `HIGRESS_CONFORMANCE_VERSION`，执行 `make gateway-conformance-test`；
...
- `gateway-conformance-test`：基于 Gateway API 官方 Suite 的 Higress Gateway API 集成测试；
...
3. 补全现有空 `gateway-conformance-test` job，使所有 PR 默认运行完整 `GATEWAY-HTTP`。
4. Make target 固定传入三个 Core features，不依赖 GatewayClass status 推断。
5. 保存失败测试名、Gateway/Route status、events 和日志。

验收：`gateway-conformance-test` 不再空成功，所有 PR 默认执行，失败会使该 workflow 失败，并且能稳定得到 33 个 Core 测试的结果。

### 阶段 B：按规范修复 Core

按以下类别归档失败，不为测试名称写特判：

1. GatewayClass Accepted 和 observedGeneration。
...
1. 固定测试入口的 Core supported features，不改变生产 status。
2. 确认普通 PR 不需要额外参数即可运行完整 Profile。
3. checkout 一个不可变 Higress release tag。
4. 执行与 PR 完全相同的 `make gateway-conformance-test`，只覆盖 implementation version。
5. 不修改生成的 YAML。
...
最终直接通过 ACK LoadBalancer `47.238.232.74` 运行，未使用 `kubectl port-forward`。原始报告位于 `out/gateway-api-conformance/ack-same-namespace-e2d36b54-report.yaml`，SHA-256 为 `45b5e7c19d2b7eb20f3444fedbfb468fcb4b1a142c383c2e55ca2a3436d0ca06`，结果为：

```yaml
gatewayAPIVersion: v1.4.0
gatewayAPIChannel: standard
implementation:
  version: e2d36b54c1825bdacb2485c3ef1cf68ad5f588f0
profiles:
- name: GATEWAY-HTTP
  core:
    result: success
...
| `HTTPRouteServiceTypes` | backend 存在性只依赖 Higress `LookupHostname`；其 GatewayContext 暂不可见时会把已有 Service 误报为 `BackendNotFound` | 保留 `LookupHostname`，仅在返回 nil 时用 Kubernetes Service informer 兜底对象存在性 | ClusterIP、headless、手工 EndpointSlice 三类请求全部通过 |

为确认同 namespace 是否可以完全避免生产代码改动，又使用 `origin/main@154782660cabb5ae8d313e6348efe747bb5e9d16` 原始 core、相同 namespace、相同 pilot/gateway 和相同 LoadBalancer 运行了一次完整对照。原始报告为 `out/gateway-api-conformance/ack-same-namespace-origin-main-report.yaml`，SHA-256 为 `40d643857073e64fc51d769fe30efd8c4dfacc9745c0b3f0b5a3481632aa4908`，结果为 `Passed: 28 / Failed: 5 / Skipped: 0`：

| 基线失败测试 | 与 namespace 的关系 | 最小必要修复 |
| --- | --- | --- |
| `GatewayClassObservedGenerationBump` | 无关；用例创建名称不同但 controllerName 相同的 GatewayClass | 按 `spec.controllerName` 认领，而不是只认固定名称 `higress` |
| `HTTPRouteHTTPSListener`、`HTTPRouteHostnameIntersection`、`HTTPRouteListenerHostnameMatching` | 无关；HTTPS 已能握手，但不相交 hostname 仍被错误路由并返回 200 | 生成 VirtualService 前计算 listener/route hostname 的有效交集 |
| `HTTPRouteServiceTypes` | 无关；已有 headless Service 被 `LookupHostname` 误判不存在 | 保留 `LookupHostname`，使用 Kubernetes Service informer 兜底判断对象是否存在 |

因此，同 namespace 能消除此前 Service/Pod 选择和 SDS 信任边界问题，但不能让当前主干零代码通过全部 Core。完整 v1.4 Core 最少仍需上述三类产品修复；没有通过修改官方 manifests、跳过测试或添加测试名特判来规避。

Gateway API v1.4.0 官方基础 manifests 本身就把所有基础 Gateway 放在 `gateway-conformance-infra`，同 namespace TLS Secret 也放在该 namespace。规范不要求 controller 必须部署在独立 namespace。对于根据每个 Gateway 自动生成的 in-cluster Service/Deployment，v1.4 的 Standard GEP-1762 反而要求 namespace-scoped 生成资源与 Gateway 位于同一 namespace。Higress 当前使用预部署共享 gateway，并非按 Gateway 自动创建一套数据面资源；把测试用共享 gateway 放到 `gateway-conformance-infra` 不违反该要求，也不改变官方对跨 namespace Route、backend 和 Secret 的 `allowedRoutes`/`ReferenceGrant` 验证。

最终使用的镜像如下。只有 `higress-core` 使用当前提交重新构建的镜像；pilot 和 gateway 均保持与仓库其他集成测试一致的统一 tag，没有为 Gateway API 用例制作专用数据面镜像：

| 组件 | tag | 运行结果 |
| --- | --- | --- |
| higress-core | `e2d36b54c1825bdacb2485c3ef1cf68ad5f588f0`，digest `sha256:bb8db41ffb45f022ee12aab268ee6cdc2afb10946bc6c3eff3101ceb0f169ec1` | Ready，0 次重启 |
| pilot | `de2c9628294f51b13c4a70b3a862b4372890797a` | Ready，0 次重启 |
| gateway | `481184afc44176eb23d64e0011dc3ea1ae6a410c` | Ready，0 次重启 |

缩减生产改动并移除 `GatewayClass.status.supportedFeatures` 后，使用上述 commit 镜像再次全量执行，仍得到 `Passed: 33 / Failed: 0 / Skipped: 0`。该轮直接访问 LoadBalancer，且 Suite 日志明确显示 features 来自命令行参数。`go test` 输出中其他 profile 的 `SKIP` 是未选择的 Extended、Mesh、GRPC、TLSRoute 等测试，不计入 `GATEWAY-HTTP Core`；官方报告中的 Core `Skipped` 为 0。
...
- `make gateway-conformance-test` 真正执行 v1.4.0 官方 Suite。
...
- 测试入口只声明 `Gateway`、`HTTPRoute`、`ReferenceGrant`，不修改生产 `supportedFeatures` 状态。
