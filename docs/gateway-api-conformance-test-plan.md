...
| `HTTPRouteServiceTypes` | 无关；无 Endpoint 的 headless Service 被误判不存在 | 使用 Kubernetes Service informer 判断 Service 对象是否存在 |
