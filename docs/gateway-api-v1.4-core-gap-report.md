...
3. Service backend 是否存在被错误地通过 Istio 服务发现结果判断，而不是通过 Kubernetes Service 对象判断。
...
| `HTTPRouteServiceTypes` | 已存在的普通、Headless、手工 EndpointSlice Service 都应得到 `ResolvedRefs=True`，并可在端点存在时访问 | 手工 EndpointSlice 场景被标记 `BackendNotFound` | 使用 `LookupHostname` 查询 Istio 服务发现模型；没有自动生成端点时可能查不到模型对象，即使 Kubernetes Service 实际存在 |
...
### 5.3 使用 Kubernetes Service informer 判断对象是否存在

文件：`pkg/ingress/kube/gateway/istio/conversion.go`

- Service backend 使用 `namespace/name` 从 `ctx.Services` informer 集合读取 Kubernetes Service；
- 只有 Service 对象不存在时才设置 `BackendNotFound`；
- EndpointSlice 和端点健康状态仍由现有服务发现/数据面链路处理。

这与当前 Istio 1.27 fork 中 Service backend 的存在性判断方式一致。
...
| Service informer 判断 | 已存在但暂时无端点、Headless 或使用手工 EndpointSlice 的 Service 不再被标记 `BackendNotFound` | 主要改变状态准确性。无可用端点时请求仍可能返回 503，修复不会凭空生成端点，风险低 |
