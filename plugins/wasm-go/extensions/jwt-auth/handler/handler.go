...
func OnHTTPRequestHeaders(ctx wrapper.HttpContext, config cfg.JWTAuthConfig, log log.Log) types.Action {
	...
	decision := verifyConsumers(config, log)
	if decision.remoteConsumer != nil {
		return fetchRemoteJWKsAndVerify(ctx, decision.remoteConsumer, config, log)
	}
	return decision.action()
}

func fetchRemoteJWKsAndVerify(_ wrapper.HttpContext, consumer *cfg.Consumer, config cfg.JWTAuthConfig, log log.Log) types.Action {
	err := fetchRemoteJWKs(consumer, log, func() {
		completeAuthenticationAfterRemoteFetch(config, log)
	})
	if err != nil {
		log.Warnf("failed to dispatch remote jwks fetch, consumer:%s, reason:%s", consumer.Name, err.Error())
		return actionAfterRemoteFetch(config, log)
	}
	return types.HeaderStopAllIterationAndWatermark
}

func completeAuthenticationAfterRemoteFetch(config cfg.JWTAuthConfig, log log.Log) {
	decision := decisionAfterRemoteFetch(config, log)
	_ = decision.action()
	if decision.resume {
...
func actionAfterRemoteFetch(config cfg.JWTAuthConfig, log log.Log) types.Action {
	return decisionAfterRemoteFetch(config, log).action()
}

func decisionAfterRemoteFetch(config cfg.JWTAuthConfig, log log.Log) authDecision {
	for {
		decision := verifyConsumers(config, log)
		if decision.remoteConsumer == nil {
			return decision
		...
		if err == nil {
			return authDecision{action: func() types.Action { return types.HeaderStopAllIterationAndWatermark }}
		}
...
func verifyConsumers(config cfg.JWTAuthConfig, log log.Log) authDecision {
	...
	for i := range config.Consumers {
		...
		verified, err := consumerVerify(consumer, time.Now(), header, log)
		if err != nil {
