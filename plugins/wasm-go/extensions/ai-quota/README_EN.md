...
> **Redis Cluster compatibility**: when `group` is set, Phase 1/2 use Lua scripts that operate on both the group and consumer keys atomically. Redis Cluster requires all keys in a multi-key operation to land on the same slot, otherwise it returns `CROSSSLOT`. ai-quota automatically wraps `redis_key_prefix` as a hash tag (`chat_quota:` → `{chat_quota}`), so all quota keys share the same slot. **Trade-off**: all ai-quota traffic concentrates on a single slot, losing Cluster sharding. If quota traffic is high and you need horizontal sharding, use distinct `redis_key_prefix` values per tenant (e.g., `tenant_a_quota:`, `tenant_b_quota:`).
>
