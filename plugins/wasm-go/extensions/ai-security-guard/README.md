...
| `riskAction` | string | optional | block | 风险处置动作，取值为 `block` 或 `mask`。`block` 表示按风险等级阈值拦截请求，`mask` 表示当 API 返回脱敏建议时使用脱敏内容替换敏感字段；在 `checkResponse: true` 且 OpenAI `text_generation` 响应可写时，也会尝试改写响应内容。注意：脱敏功能仅适用于 MultiModalGuard 模式 |
...
- 对于风险处置动作（riskAction）：
    ...
    - `mask`: 当 API 返回 `Suggestion=mask` 时使用脱敏内容替换敏感字段，当 `Suggestion=block` 时仍会拦截；开启 `checkResponse: true` 时，OpenAI `text_generation` 响应也可按脱敏结果改写
...
当 `action: MultiModalGuard`、`riskAction: mask` 且 `checkResponse: true` 时，如果内容安全服务对 OpenAI `text_generation` 响应返回 `Suggestion=mask` 并携带非空 `Ext.Desensitization`，插件会用脱敏文本改写响应内容：
...
- 非流式响应会改写 `responseContentJsonPath` 或实际命中的兜底路径，并移除可能过期的 `content-length`
- 流式 SSE 响应只改写当前已缓冲的 `data:` JSON payload，使用每个 chunk 实际命中的可写路径，保留 SSE framing、`[DONE]`、空 chunk 与非内容字段
- 如果脱敏文本为空，或 JSON/SSE 改写失败，插件会按现有响应拒答路径 fail-closed，避免转发原始敏感内容
- `protocol: original` 不支持响应脱敏；响应阶段命中 `RiskMask` 时会按拒答处理。流式 original 协议会返回终止 SSE 帧，`data:` payload 为原始协议拒答 JSON，不使用 OpenAI chunk 模板
- TextModerationPlus 与 MultiModalGuard 共用文本响应 handler，但非 MultiModalGuard action 的 `EvaluateRisk` 不会产生 `RiskMask`，因此不会进入响应脱敏路径
- 非流式响应分片会按 UTF-8 字符边界对齐，避免将多字节字符拆开后提交给内容安全服务
...
- `ai_sec_response_mask`: 模型回答成功完成响应脱敏的请求数（每个网关响应最多计数一次）
...
| `mask` | 该次提交命中风险且 `Action=Mask`，安全服务返回了脱敏文本并用于改写请求体或响应体 |
...
| `response mask` | 响应阶段命中 mask，响应体或当前缓冲的 SSE chunk 已被脱敏文本改写；同一响应内后续 pass 不会覆盖该状态 |
