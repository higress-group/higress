...
func TestParseConfig(t *testing.T) {
	test.RunGoTest(t, func(t *testing.T) {
		...
		t.Run("consumer specific config", func(t *testing.T) {
			...
			require.Equal(t, "default", d1.Source)
			d2 := securityConfig.ResolveRequestCheckService("aaa")
			require.True(t, d2.Enabled)
			require.Equal(t, "llm_query_moderation_1", d2.Service)
			require.Equal(t, "consumer", d2.Source)
			d3 := securityConfig.ResolveResponseCheckService("bb")
			require.True(t, d3.Enabled)
			require.Equal(t, "llm_response_moderation", d3.Service)
			require.Equal(t, "default", d3.Source)
			d4 := securityConfig.ResolveResponseCheckService("bbb-prefix-test")
			require.True(t, d4.Enabled)
			require.Equal(t, "llm_response_moderation_1", d4.Service)
			require.Equal(t, "consumer", d4.Source)
