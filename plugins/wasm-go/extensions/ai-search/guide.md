...
promptTemplate: | # 可以不用配置模版，使用内置的也可以
  ...
  - For listing-type questions (e.g., listing all flight information), try to keep the answer to within 10 points and inform the user that they can check the search source for complete information. Prioritize providing the most comprehensive and relevant matches; do not volunteer information missing from the search results unless necessary.
